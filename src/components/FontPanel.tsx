        ...
        <div className={styles.modalBody}>
          {missingUppercase.length > 0 && (
            <div className={styles.modalSection}>
              ...
              <p className={styles.modalWarningText}>
                All 26 uppercase letters (A–Z) are recommended for a complete font. The exported file will work, but characters without glyphs will render as the fallback box.
              </p>
            </div>
          )}
          ...
          {strokeCount > 0 && (
            <div className={styles.modalSection}>
              ...
              <p className={styles.modalWarningText}>
                {strokeCount} glyph{strokeCount === 1 ? '' : 's'} use strokes instead of filled paths and will not be included in the export. Convert strokes to outlines in your SVG editor to include them.
              </p>
            </div>
          )}
        </div>
...
export default function FontPanel() {
  ...
  // Close on Escape (only when no modal is open)
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
  ...
  const handleExport = (format: 'ttf' | 'otf') => {
    ...
    // Hard blocked — no glyphs at all
    if (validation.uploadedCount === 0) return

    // All good — export immediately
    if (validation.valid) {
      doExport(format)
      return
    }

    // Has glyphs but missing uppercase — show confirmation modal
    if (validation.canExportWithWarning) {
  ...
  return (
    <>
      {/* Confirmation modal — rendered outside the panel so it covers everything */}
      {pendingExport && (
        <ConfirmModal
      ...
      <div style={{ position: 'relative' }}>
        ...
        >
          <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
            ...
            <path d="M6.5 1v1.5M6.5 10v1.5M1 6.5h1.5M10 6.5h1.5M2.7 2.7l1 1M9.3 9.3l1 1M9.3 2.7l-1 1M2.7 9.3l-1 1" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/>
          </svg>
          Font settings
          <svg
                ...
                <div className={styles.content}>
                  <div className={styles.exportSection}>
                    ...
                    <div className={styles.validationBox}>
                      ...
                      <div className={styles.validationRow}>
                        <span>Total glyphs uploaded</span>
                        ...
                        <div className={styles.warningItem}>
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ marginBottom: 6 }}>
                              {validation.missingUppercase.length} uppercase letter{validation.missingUppercase.length === 1 ? '' : 's'} missing — export will require confirmation.
                            </div>
                            <div className={styles.missingGrid}>
                    ...
                    {validation.strokeGlyphs.length > 0 && (
                      <div className={styles.warningList}>
                        <div className={styles.warningItem}>
                          ...
                          {validation.strokeGlyphs.length} glyph{validation.strokeGlyphs.length === 1 ? '' : 's'} with strokes will be skipped during export.
                        </div>
                      </div>
                    )}
                    ...
                    <p style={{ fontSize: 11, color: 'var(--text-tertiary)', lineHeight: 1.55 }}>
                      All 26 uppercase letters (A–Z) must be filled for a clean export. Exporting with missing glyphs requires confirmation. Only filled paths are included — strokes are skipped.
                    </p>
                  </div>
                </div>
              )}

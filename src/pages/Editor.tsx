...
export default function Editor({ onBack }: EditorProps) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (ctrl && e.key === 'p') { e.preventDefault(); setIsPreview(p => !p) }
    }
  ...
  if (isPreview) {
    return (
      <div className={styles.root}>
        <Navbar onBack={onBack} onPreview={handleTogglePreview} isPreview={true} />
  ...
  return (
    <div className={styles.root}>
      <Navbar onBack={onBack} onPreview={handleTogglePreview} isPreview={false} />
      <div className={styles.body}>
